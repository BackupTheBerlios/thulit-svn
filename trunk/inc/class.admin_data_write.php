...
class ADMIN_DATA_WRITE{
    ...
    function write($edit_id, $parent_id='0'){
        if ($edit_id!='0'){
            ...
            if (isSet($this->to_write['class'])){$q.= ", class='".addslashes($this->to_write['class'])."'";}
            if (isSet($this->to_write['view_level'])){$q.= ", view_level='".addslashes($this->to_write['view_level'])."'";}
            if (isSet($this->to_write['edit_level'])){$q.= ", edit_level='".addslashes($this->to_write['edit_level'])."'";}
